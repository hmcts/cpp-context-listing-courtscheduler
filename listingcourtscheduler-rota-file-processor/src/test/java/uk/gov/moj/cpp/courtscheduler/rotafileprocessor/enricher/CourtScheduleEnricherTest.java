...
//        when(sessionsService.findByCourtRoomIdAndSessionDateAndBusinessTypeAndCourtSession(anyString(), any(LocalDate.class), anyString(), anyString())).thenReturn(null);
        ...
        final CourtSchedule courtSchedule = courtScheduleEnricher.build(listingProfile, LocalDate.of(2019, 10, 1), emptyList(), requester);
...
//        when(sessionsService.findByCourtRoomIdAndSessionDateAndBusinessTypeAndCourtSession(anyString(), any(LocalDate.class), anyString(), anyString())).thenReturn(new CourtScheduleMatcherInfo(courtScheduleId, courtRoom.getOucode(), Calendar.getInstance().getTime()));
        ...
        final CourtSchedule courtSchedule = courtScheduleEnricher.build(listingProfile, LocalDate.of(2019, 10, 1), List.of(CourtSchedule.CourtScheduleBuilder.courtSchedule()
    ...
    void shouldBuildNewCourtScheduleWithCourtRoomDetailsNotPresentLogMessages() {
        ...
        courtScheduleEnricher.build(listingProfile, LocalDate.of(2019, 10, 1), emptyList(), requester);
    ...
    void shouldBuildNewCourtScheduleWithSessionAllocationDetailsNotPresentLogMessages() {
        ...
        final CourtSchedule courtSchedule = courtScheduleEnricher.build(listingProfile, sessionDateLD, emptyList(),  requester);
        assertThat(courtSchedule.getListingProfileId(), is("CS2129874"));
        assertThat(courtSchedule.getSessionDate(), is(LocalDate.of(2019, 10, 01)));
        assertThat(courtSchedule.getPanel(), is("ADULT"));
        assertThat(courtSchedule.getBusinessType(), is("DVB"));
