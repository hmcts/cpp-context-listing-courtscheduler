...
class RotaDataEnricherTest {
    ...
    void shouldEnrichListingWithCppReferenceData() throws IOException {
        ...
        final List<CourtSchedule> courtScheduleList = List.of(copyOf(courtSchedule)
    ...
    void updateExistingCourtScheduleShouldUseRefdataTimesForAllDay() {
        ...
        assertThat(updated.getSessionStartTime(), is(DateUtils.combineDateAndTime(sessionDate, "09:15").toInstant().atOffset(java.time.ZoneOffset.UTC)));
        assertThat(updated.getSessionEndTime(), is(DateUtils.combineDateAndTime(sessionDate, "16:30").toInstant().atOffset(java.time.ZoneOffset.UTC)));
    ...
    void updateExistingCourtScheduleShouldUseDefaultAllDayTimesWhenRefdataMissing() {
        ...
        final CourtSchedule updated = result.get("L1");
        assertThat(updated.getCourtSession(), is(ALL_DAY_SESSION));
        assertThat(updated.getSessionStartTime(), is(DateUtils.combineDateAndTime(sessionDate, "10:00").toInstant().atOffset(java.time.ZoneOffset.UTC)));
        assertThat(updated.getSessionEndTime(), is(DateUtils.combineDateAndTime(sessionDate, "17:00").toInstant().atOffset(java.time.ZoneOffset.UTC)));
    }
    ...
    private CourtSchedule copyOf(final CourtSchedule source) throws JsonProcessingException {
        return objectMapper.readValue(objectMapper.writeValueAsString(source), CourtSchedule.class);
    }
}
