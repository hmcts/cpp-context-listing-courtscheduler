...
public final class CourtSchedulerConverter {
    ...
    public static CourtSchedule convert(final uk.gov.moj.cpp.courtscheduler.persist.entity.CourtSchedule courtScheduleEntity) {
        ...
        final CourtSchedule.CourtScheduleBuilder courtScheduleBuilder = new CourtSchedule.CourtScheduleBuilder()
                ...
                .withAllDaySplit(Boolean.TRUE.equals(isAllDaySplit))
                ...
                .withIsOverbookingAllowed(courtScheduleEntity.isOverbookingAllowed())
                .withIsDraft(courtScheduleEntity.isDraft())
                .withMinHearingTime(sessionTimeFormatter(courtScheduleEntity.getSessionStartTime()))
                .withMaxHearingTime(sessionTimeFormatter(courtScheduleEntity.getSessionEndTime()));
    ...
    public static CourtSchedule convertForOverbooking(
        ...
        return new CourtSchedule.CourtScheduleBuilder()
                .withCourtScheduleId(courtScheduleEntity.getCourtScheduleId())
                .withIsOverbookingAllowed(Boolean.TRUE.equals(courtScheduleEntity.isOverbookingAllowed()))
                .withActive(courtScheduleEntity.isActive())
                .build();
    }
    ...
    public static CourtSchedule convert(final uk.gov.moj.cpp.courtscheduler.persist.entity.CourtSchedule courtScheduleEntity,
        ...
        final Instant minHearingTime = allocatedListings.stream()
                .map(AllocatedListingEachBooked::getHearingStartTime)
                .min(Instant::compareTo)
                .orElse(null);
        final Instant maxHearingTime = allocatedListings.stream()
                .max(comparing(AllocatedListingEachBooked::getHearingStartTime))
                .map(AllocatedListingEachBooked::getHearingStartTime)
                .orElse(null);
        if (minHearingTime != null) {
            courtScheduleBuilder.withMinHearingTime(sessionTimeFormatter(minHearingTime));
        }
        if (maxHearingTime != null) {
            courtScheduleBuilder.withMaxHearingTime(sessionTimeFormatter(maxHearingTime));
        }
        ...
        if (Boolean.TRUE.equals(courtScheduleEntity.isSupportAdSplit()) && ALL_DAY.equals(courtScheduleEntity.getCourtSession())) {
            ...
            final Instant sessionStartInstant = courtScheduleEntity.getSessionStartTime();
            final Instant afternoonCutoffInstant = combineDateAndTime(courtScheduleEntity.getSessionDate(), DEFAULT_AFTERNOON_START_TIME).toInstant();
            allocatedListings
                    .forEach(eachBooked -> {
                        if ((eachBooked.getHearingStartTime().isAfter(sessionStartInstant) || eachBooked.getHearingStartTime().equals(sessionStartInstant))
