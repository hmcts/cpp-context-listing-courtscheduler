...
public class MeridianHelper {
    private static final Set<Meridian> AM_MERIDIAN = range(TWELVE_AM, TWELVE_PM);
    private static final Set<Meridian> PM_MERIDIAN = range(ONE_PM, FIVE_PM);
    ...
    public static String getMeridian(final ZonedDateTime hearingDaySittingDay) {
        ...
        final boolean isAmMeridian = AM_MERIDIAN.stream().anyMatch(meridian -> checkMeridian(meridian.getValue(), hour));
        final boolean isPmMeridian = PM_MERIDIAN.stream().anyMatch(meridian -> checkMeridian(meridian.getValue(), hour));
